...
object ImportTool extends ScorexLogging {
  def main(args: Array[String]): Unit = {
    ...
    val history = HistoryWriterImpl(settings.blockchainSettings.blockchainFile, new ReentrantReadWriteLock(true)).get
    ...
    (persistedHeight + 1 to 600000).foldLeft(Option.empty[Block]) {
      case (prevBlock, height) =>
        ...
        BlockDiffer.fromBlock(settings.blockchainSettings.functionalitySettings, state, prevBlock.map(_.timestamp))(block.get) match {
          case Right(diff) =>
